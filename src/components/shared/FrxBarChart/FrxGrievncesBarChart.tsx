...
class FrxPaBarChart extends React.Component<FrxBarChartProps>{
    componentDidMount = () => {
        ...
        var _data = data.sort(function (a: any, b: any) { return a.key === 'open' ? -1 : 1 })
        ...
        svg.selectAll("mybar")
            ...
            .enter()
            .append("rect")
            .attr("x", function (d: any): any { return x(d.key); })
            .attr("width", x.bandwidth())
            .attr("fill", function (d: any) { return d.key === 'open' ? "#59B35E" : "#F65A1C" && d.key === 'withdrawn' ? "#F65A1C" : "#666666" })
    ...
    render() {
        ...
        return (
            <div className={this.props.isSelected ? "barDiag Selected" : "barDiag unSelected"}>
                ...
                <Box component="span" display="block">
                    <Button variant="outlined" disabled={isDisabled} onClick={() => { this.props.itemClicked() }} style={{ borderRadius: 25, paddingLeft: 30, paddingRight: 30, fontSize: 18, fontWeight: 'bold' }}>{this.props.title.substr(0, 3).toLocaleUpperCase()}</Button>
                    <ToolTip>
