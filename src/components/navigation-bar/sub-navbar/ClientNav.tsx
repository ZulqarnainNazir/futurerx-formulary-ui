...
export default class ClientNav extends React.Component<any, any> {  
  handleClick = () => {
  ...
  render() {
    ...
    return (
      <div>
        ...
        <div className="clients_div removeMargin padding15">
          {clients.map((client, index) => (
            <Grid container id={client.id} key={client.id}>
              <Grid item className="circle" xs={1}>
                <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                <circle cx="15" cy="15" r="14.5" fill="white" stroke="#CCCCCC" stroke-dasharray="2 2"/>
                </svg>
              </Grid>
              <Grid item xs={11} className="clientName" onClick={()=>this.handleClick()}>
                {client.Name}
              </Grid>
            </Grid>
          ))}
