        ...
        if(this.state.activeTabIndex !== 2) {
          if(this.state.posSettingsStatus.covered) {
            if(element.covered_place_of_services) {
              let cprsArray = element.covered_place_of_services.split(",").map(e => e.trim().toLowerCase());
    
              let chFilterSettings = this.state.posSettings.filter(e => e.isChecked).map(e => e.place_of_service_type_name.toLowerCase());
            ...
            if(element.not_covered_place_of_services) {
              let ncgendersArray = element.not_covered_place_of_services.split(",").map(e => e.trim().toLowerCase());
