...
import { getAllFormularyVersions, deleteFormulary, deleteFullFormulary } from "../../../../../../../redux/slices/formulary/setup/setupService";
...
export default class DeletePopup extends React.Component<any, any> {
    ...
    onDeleteVersionClicked = async () => {
        if (this.props.currentFormulary) {
            try {
                let response = await deleteFormulary(this.props.currentFormulary.id_formulary);
    ...
    onDeleteFullFormularyClicked = async () => {
        if (this.props.currentFormulary) {
            try {
                ...
                let response = await getAllFormularyVersions(payload, this.props.currentFormulary.id_base_formulary, 10000, 0);
                if (response && response.status && response.status === 200) {
    ...
    render() {
        return (
            <div className="popup-container">
                <Grid container>
                    <Grid item xs={12}>
                        <p>Do you want to delete the version or the full formulary: <span>{this.props?.currentFormulary?.formulary_info?.formulary_name ? this.props?.currentFormulary?.formulary_info?.formulary_name : ''} ?</span></p>
                    </Grid>
                    <Grid item xs={12}>
                        <div className="action-btn">
                            <Button label="Cancel" htmlFor="upload-file" className="upload-button cancel-btn" onClick={this.onCancelClicked} />
                            <Button label="Delete Version" htmlFor="upload-file" className="upload-button save-btn" onClick={this.onDeleteVersionClicked} />
                            <Button label="Delete Full Formulary" htmlFor="upload-file" className="upload-button save-btn" onClick={this.onDeleteFullFormularyClicked} />
                        </div>
                    </Grid>
                </Grid>
            </div>
