...
function GroupHeader(props: any) {
    ...
    props.deleteGroupDescription({ lob_type: lob_type, pathParams: pathParams }).then((json) => {
      props.getStGrouptDescriptions({ lob_type: lob_type, pathParams: props.saveGdm.formulary_id }).then((json) => {
        ...
        let id_st_group_description =
          groupListLength > 0 ? groupList.filter((val) => val.is_archived === false)[0].id_st_group_description : 0;
        ...
        props
          .getStGrouptDescriptionVersions({ lob_type: lob_type, pathParams: id_st_group_description })
          .then((json) => {
  ...
  const archiveGroup = (e: any, param: any) => {
    ...
    props.archiveGroupDescription({ lob_type: lob_type, pathParams: pathParams }).then((json) => {
      props.getStGrouptDescriptions({ lob_type: lob_type, pathParams: props.saveGdm.formulary_id });
      props
        .getStGrouptDescriptionVersions({ lob_type: lob_type, pathParams: props.saveGdm.current_group_id })
        .then((json) => {
          const response = json.payload.data;
          const verLength = Object.keys(response).length;
          const isEditable = response[verLength - 1].is_setup_complete;
      ...
      <div className="item" onClick={toggleShowViewAll}>
        <svg width="11" height="11" viewBox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            ...
            fill={panelColor === "-green" ? "#219653" : "#f65a1c"}
          />
        </svg>
        Version History
      </div>
