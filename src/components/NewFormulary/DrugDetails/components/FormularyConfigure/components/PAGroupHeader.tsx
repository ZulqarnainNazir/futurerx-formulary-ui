...
function PAGroupHeader(props: any) {
  ...
  const onChange = (e) => {
    ...
    if (verLength > 0 && selectedVersion != "") {
      ...
      const latestVerion =
        verLength > 0 ? props.version[Number(selectedVersion.split(" ")[1]) - 1]?.id_pa_group_description : 0;
      setPanelColor(isEditable ? "-green" : "");
  ...
  const deleteGroup = (e: any, param: any) => {
    props
      .deleteGroupDescription({ current_group_des_id: selectedVersionId, lob_type: props.formulary_lob_id })
      .then((json) => {
        let apiDetails = {};
        apiDetails["lob_type"] = props.formulary_lob_id;
  ...
  const archiveGroup = (e: any, param: any) => {
    props
      .archiveGroupDescription({ current_group_des_id: selectedVersionId, lob_type: props.formulary_lob_id })
      .then((json) => {
        let apiDetails = {};
        apiDetails["lob_type"] = props.formulary_lob_id;
        apiDetails["pathParams"] = "/" + props.client_id;
  ...
  const newVersionGroup = (e: any, param: any) => {
    ...
    props
      .newVersionGroupDescription({ current_group_des_id: selectedVersionId, lob_type: props.formulary_lob_id })
      .then((json) => {
        let apiDetails = {};
        apiDetails["lob_type"] = props.formulary_lob_id;
        apiDetails["pathParams"] = "/" + props.client_id;
