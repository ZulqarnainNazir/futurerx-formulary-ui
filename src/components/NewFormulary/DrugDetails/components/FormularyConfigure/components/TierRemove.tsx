...
class TierRemove extends React.Component<any, tabsState> {
  ...
  onSelectedTableRowChanged = (selectedRowKeys) => {
    ...
    if (selectedRowKeys && selectedRowKeys.length > 0) {
      this.state.selectedDrugs = selectedRowKeys.map(tierId => this.state.drugData[tierId - 1]['formulary_drug_id'] ? this.state.drugData[tierId - 1]['formulary_drug_id'] : this.state.drugData[tierId - 1]['drug_id']);
    }
  }
